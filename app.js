...
function render() {
  ...
  $("#app").innerHTML = `
    ...
    <div class="workspace">
      ...
      <div class="demo-banner">${icon("info")}<span><strong>Public fictional demo.</strong> Edits here stay in this browser. For online AI, public shared saving, and Gmail reminders, <a href="https://aftercare-muj-podiatrist.muddq-finances.workers.dev">open the online app</a>. No login; anyone with the link can edit. Invented cases only.</span><button data-action="about">How it works ${icon("arrow")}</button></div>
...
function about() {
  confirmation({
    ...
    content: `<div class="about-copy"><p><strong>What works here:</strong> code-based cases, your milestone dates, recorded outcomes, overdue lists, a weekly brief, calendar exports, and a local rule-based assistant.</p><p><strong>Online AI app:</strong> <a href="https://aftercare-muj-podiatrist.muddq-finances.workers.dev">Open the public shared workspace</a> for real AI, shared online saving, and hosted Gmail reminders. No login is required; anyone with the link can view and edit the fictional cases. Email administration stays private. It is separate from this demo; these browser records are not automatically uploaded.</p><p><strong>Public snapshot:</strong> the fictional case CSV is stored publicly in the GitHub repository, including Git history. New browsers load it on their first visit. Use “Load published cases” to replace this browser’s copy with the published snapshot.</p><p><strong>Your edits here:</strong> changes stay in this browser. The older local Mac pilot can publish an explicitly confirmed fictional CSV snapshot. The hosted online app does not publish records to GitHub. Browser backups are unencrypted.</p><p><strong>What neither prototype is:</strong> an approved clinical record, EHR integration, or automatic attendance feed. Record attendance yourself.</p><p><strong>Codes are not automatic de-identification.</strong> Dates and a re-identifiable code can still be protected health information. Use invented cases only. Real use needs an appropriate deployment and de-identification/compliance review.</p><p>All example schedules are fictional interface demonstrations, not postoperative protocols. Clinical decisions remain yours.</p></div>`,
    label: "Got it",
    onConfirm: () => {}
  });
}
