import {
  ...
  milestoneState, isOpen, allTasks, attentionTasks, weeklyTasks, nextMilestone,
...
function taskRow({ patient, milestone }) {
  return `<button class="task-row" data-action="patient" data-id="${patient.id}">
    ${avatar(patient)}
    ...
    ${badge(milestone)}${icon("chevron", "row-arrow")}
  </button>`;
}
...
function render() {
  ...
  $("#app").innerHTML = `
    <aside class="sidebar">
      ...
      <nav aria-label="Main navigation">${nav.map(([view, symbol, label]) => `<button class="nav-item ${state.view === view ? "active" : ""}" data-action="navigate" data-view="${view}" ${state.view === view ? 'aria-current="page"' : ""}>${icon(symbol)}${label}${view === "overview" && attention.length ? `<span class="nav-count">${attention.length}</span>` : ""}</button>`).join("")}</nav>
...
function renderOverview(attention, week) {
  ...
  const completed = allTasks(state.patients).filter(({ milestone: m }) => m.status === "completed" && m.completedDate >= week.start && m.completedDate <= week.end).length;
  ...
  return `
    <section class="page-heading"><div><p class="eyebrow">${formatDate(state.today, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</p><h1>Your week, in good hands.</h1><p>A clear view of recovery. A little more peace of mind.</p></div><button class="button primary" data-action="new-case">${icon("plus")} Add a case</button></section>
    <section class="stats-grid" aria-label="Workspace summary">
      ...
      <button class="stat-card" data-action="filter" data-filter="week"><span class="stat-label">Still due this week ${icon("calendar")}</span><span class="stat-number">${due.length}<span class="stat-unit">milestone${due.length === 1 ? "" : "s"}</span></span><span class="stat-description">${formatDate(state.today)} – ${formatDate(week.end)} ${icon("arrow")}</span></button>
      ...
      <div class="stat-card"><span class="stat-label">Completed this week ${icon("check")}</span><span class="stat-number">${completed}<span class="stat-unit">milestone${completed === 1 ? "" : "s"}</span></span><span class="stat-description">Every follow-through counts <span class="tiny-leaf">${icon("leaf")}</span></span></div>
    </section>
    <div class="overview-grid">
      <section class="panel attention-panel"><div class="panel-heading"><div><h2>A little attention goes a long way.</h2><p>Start here. Nothing important gets buried.</p></div><span class="small-icon amber">${icon("clock")}</span></div>
        <div class="tabs" role="group" aria-label="Milestone list"><button data-action="filter" data-filter="all" class="${state.filter !== "week" ? "selected" : ""}" aria-pressed="${state.filter !== "week"}">Needs attention <span>${attention.length}</span></button><button data-action="filter" data-filter="week" class="${state.filter === "week" ? "selected" : ""}" aria-pressed="${state.filter === "week"}">This week <span>${due.length}</span></button></div>
      ...
      <section class="assistant-card"><div class="assistant-orbit" aria-hidden="true"><div>${icon("spark")}</div><span class="orbit-dot one"></span><span class="orbit-dot two"></span></div><span class="eyebrow">MEET YOUR SECOND SET OF EYES</span><h2>Less remembering.<br>More caring.</h2><p>Ask what’s coming up, find a loose end, or turn your plan into a timeline.</p><button class="assistant-example" data-action="assistant" data-prompt="What needs my attention?">“Who needs my attention?” ${icon("arrow")}</button><button class="button light" data-action="assistant">${icon("spark")} Let's take a look</button><span class="assistant-disclosure">Local demo assistant · no AI model connected</span></section>
    </div>
...
function renderPatients() {
  return `<section class="page-heading"><div><p class="eyebrow">THE BIG PICTURE</p><h1>Every recovery, in view.</h1><p>Patient codes. Clear next steps. No mental juggling.</p></div><button class="button primary" data-action="new-case">${icon("plus")} Add a case</button></section>
...
function renderWeek() {
  ...
  return `<section class="page-heading"><div><p class="eyebrow">YOUR WEEKLY BRIEF</p><h1>A plan for the days ahead.</h1><p>Planned milestones—not a live appointment calendar.</p></div><div class="heading-actions"><button class="button secondary" data-action="calendar">${icon("download")} Calendar</button><button class="button primary" data-action="print">${icon("print")} Print brief</button></div></section>
...
function showPatient(id) {
  ...
  $("#patient-dialog").innerHTML = `<div class="drawer-top"><span class="eyebrow">RECOVERY TIMELINE</span><button class="icon-button" data-action="close" aria-label="Close patient">${icon("close")}</button></div>
    ...
    ${[...patient.milestones].sort((a, b) => a.date.localeCompare(b.date)).map(m => {
      ...
      ${isOpen(m) ? `<div class="milestone-actions"><button class="button small secondary" data-action="milestone" data-patient="${patient.id}" data-id="${m.id}" data-operation="complete">${icon("check")} Complete</button><button class="text-button" data-action="reschedule" data-patient="${patient.id}" data-id="${m.id}">Reschedule</button><details class="action-menu"><summary aria-label="More actions for ${esc(m.label)}">•••</summary><div>${m.status !== "missed" && m.date <= state.today ? `<button data-action="milestone" data-patient="${patient.id}" data-id="${m.id}" data-operation="miss">Record confirmed no-show</button>` : ""}<button data-action="milestone" data-patient="${patient.id}" data-id="${m.id}" data-operation="contact">Log outreach attempt</button><button data-action="milestone" data-patient="${patient.id}" data-id="${m.id}" data-operation="cancel">Cancel milestone</button></div></details></div>` : ""}
      ${m.history.length ? `<details class="history"><summary>Activity history (${m.history.length})</summary><ul>${m.history.map(h => `<li>${formatDate(h.on)} · ${esc(({ complete: "Marked completed", miss: "Confirmed no-show recorded", contact: "Outreach attempt logged", cancel: "Cancelled", reschedule: `Rescheduled from ${formatDate(h.previous.date)} to ${h.newDate ? formatDate(h.newDate) : ""}` })[h.action])}</li>`).join("")}</ul></details>` : ""}</div></div>`;
...
function changeMilestone(patientId, milestoneId, operation) {
  ...
  const titles = { complete: "Mark this milestone complete?", miss: "Record a confirmed no-show?", cancel: "Cancel this milestone?", contact: "Log your outreach attempt?" };
  const explanations = {
    complete: `This records completion on ${formatDate(state.today)}. Other planned dates will not change.`,
  ...
  confirmation({
    ...
    label: operation === "complete" ? "Mark complete" : operation === "miss" ? "Record no-show" : operation === "cancel" ? "Cancel milestone" : "Log attempt",
    onConfirm: () => {
...
document.addEventListener("click", async event => {
  ...
  try {
    ...
    const target = $("#confirm-dialog").open ? $("#confirm-error") : $("#case-dialog").open ? $("#case-error") : null;
